...
class ExperienceRepositoryTest: ExperienceRepository {
    ...
    func getActiveExperiences() async -> ActiveExperiences {
...
final class ExperiencesViewModelTests: XCTestCase {
    ...
    func test_expires_today() {
        ...
        let repository = ExperienceRepositoryTest(toDoSize: 4, doneSize: 0, daysLeft: 0)
        let viewModel = ExperiencesViewModel(repository) {
            expectation.fulfill()
        }
    ...
    func test_expires_seven_days_from_now() {
        ...
        let repository = ExperienceRepositoryTest(toDoSize: 0, doneSize: 4, daysLeft: 7)
        let viewModel = ExperiencesViewModel(repository) {
            expectation.fulfill()
        }
    ...
    func test_expired_yesterday() {
        ...
        let repository = ExperienceRepositoryTest(toDoSize: 2, doneSize: 2, daysLeft: -1)
        let viewModel = ExperiencesViewModel(repository) {
            expectation.fulfill()
        }
